...
import { BezierSpline } from "src/client/objects/BezierSpline.ts";
...
export function testBezierSpline(): boolean {
    ...
    try {
        ...
        const linear = BezierSpline.createLinear(new Point(0, 0), new Point(10, 10));
        const midPoint = linear.getPointAt(0.5);
        
        if (Math.abs(midPoint.x - 5) > 0.001 || Math.abs(midPoint.y - 5) > 0.001) {
        ...
        if (Math.abs(start.x - 0) > 0.001 || Math.abs(start.y - 0) > 0.001 ||
        ...
        if (!modified || Math.abs(modified.x - 5) > 0.001 || Math.abs(modified.y - 5) > 0.001) {
            console.error("Test 3 failed: Control point modification incorrect");
            return false;
        }
        console.log("✓ Test 3 passed: Control point manipulation");
        
        // Test 4: Curve length calculation
        const line = BezierSpline.createLinear(new Point(0, 0), new Point(3, 4));
        const length = line.getLength();
        const expectedLength = 5; // 3-4-5 triangle
        
        if (Math.abs(length - expectedLength) > 0.1) {
            console.error(`Test 4 failed: Length calculation incorrect. Expected ~${expectedLength}, got ${length}`);
            return false;
        }
        ...
        if (Math.abs(derivative.x - 3) > 0.001 || Math.abs(derivative.y - 4) > 0.001) {
