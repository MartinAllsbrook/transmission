...
export class BlueNoise {
    ...
    public static generate(
        ...
        while (activeList.length > 0) {
            ...
            for (let i = 0; i < maxAttempts; i++) {
                ...
                const candidateGridX = Math.floor(
                    (candidate.x - min.x) / cellSize,
                );
                const candidateGridY = Math.floor(
                    (candidate.y - min.y) / cellSize,
                );
    ...
    private static isValidPoint(
        ...
        for (
            let y = Math.max(0, gridY - searchRadius);
            y <= Math.min(grid.length - 1, gridY + searchRadius);
            y++
        ) {
            for (
                let x = Math.max(0, gridX - searchRadius);
                x <= Math.min(grid[0].length - 1, gridX + searchRadius);
                x++
            ) {
    ...
    public static generateWithCount(
        ...
        const minDistance = Math.sqrt(
            (2 * area) / (targetCount * Math.sqrt(3)),
        );
