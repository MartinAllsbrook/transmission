...
export class Chunk extends GameObject {
    ...
    protected override start(): void {
        ...
        for (const point of treePoints) {
            ...
            const distanceToTrail = this.world.distanceToTrail(
                point.add(this.Transform.WorldPosition),
            );
        ...
        // Create jump
        const chunkCenter = this.Transform.WorldPosition.add(
            new Vector2D(this.chunkSize / 2, this.chunkSize / 2),
        );
        if (this.world.distanceToTrail(chunkCenter) < this.chunkSize / 2) {
            const jumpPosition = this.world.closestPointOnTrail(chunkCenter);
            if (jumpPosition) {
                new Jump(this, this.root, {
                    position: jumpPosition.subtract(
    ...
    private createRandomObstacle() {
        ...
        if (
            this.world.distanceToTrail(
                position.add(this.Transform.WorldPosition),
            ) < 200
        ) {
