...
export class GroundState extends PlayerState {
    ...
    private switchToGroundShifty() {
        ...
        this.body.Transform.Rotation = (this.player.ShiftyAngle * -1) +
            ExtraMath.degToRad(90); // Rotation of the body is now offset to match the now flipped shifty. 90 is for the base stance of the body
    ...
    protected physicsUpdate(deltaTime: number): void {
        // Apply gravity
        this.player.Velocity = this.player.Velocity.add(
            new Vector2D(
                0,
        ...
        const forward = Vector2D.fromAngle(
            this.board.Transform.WorldRotation - Math.PI / 2,
        );
        ...
        this.player.Velocity = this.player.Velocity.add(
            normalDirection.multiply(deltaTime * PLAYER_CONFIG.slipStrength),
        );
