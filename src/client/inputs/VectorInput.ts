...
export class VectorInput extends Input<Vector2> {
    ...
    constructor(name: string, upKeys: string[], downKeys: string[], leftKeys: string[], rightKeys: string[]) {
    ...
    private updateValue() {
        const newValue: Vector2 = {
            x: (this.rightKeysSet.size > 0 ? 1 : 0) - (this.leftKeysSet.size > 0 ? 1 : 0),
            y: (this.upKeysSet.size > 0 ? 1 : 0) - (this.downKeysSet.size > 0 ? 1 : 0),
        };
