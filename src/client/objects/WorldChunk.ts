...
export class ChunkGrid {
    ...
    constructor(
        ...
        this.splinePath.addControlPoint(new Point(size.x * 0.25, this.startingPoint));
        this.splinePath.addControlPoint(new Point(size.x * 0.75, this.endingPoint));
    ...
    private drawPath(): void {
        for (let x = 0; x < this.chunkSize.x; x++) {
            for (let y = 0; y < this.chunkSize.y; y++) {
                ...
                const distance = this.splinePath.getDistanceToPoint(position, 50);
                if (distance < 5) {
                    this.setCell(y, x, 1);
                }
            }
