...
import { CircleCollider, GameObject, Parent, Vector2D } from "framework";
...
export class Obstacle extends GameObject {
    ...
    protected override async createOwnSprites(): Promise<void> {
        ...
        for (let i = 1; i <= numLeaves; i++) {
            const leafSprite = await this.loadSprite(
                `/obsticales/tree/LeavesLayer.png`,
            );
    ...
    public override update(_deltaTime: number): void {
        this.leafSprites.forEach((leaf, index) => {
            ...
            leaf.position.y =
                ((globalThis.innerHeight - this.ScreenPosition.y) *
                        parallaxAmount + baseSpacing) * (index + 0.5);
        });

        const showDistance = 256;
        const distance = this.ScreenPosition.subtract(RootObject.offset)
            .magnitude();
