...
export class LandingParticles extends GameObject {
    ...
    public async playParticles(strength: number) {
        ...
        const numParticles = ExtraMath.clamp(
            Math.floor(strength * 20),
            20,
            100,
        );
    ...
    public override update(deltaTime: number): void {
        ...
        for (const particle of this.particles) {
            ...
            if (particle.age >= particle.lifetime) {
                ...
                particle.particle.x += Math.cos(particle.direction) * speed *
                    deltaTime;
                particle.particle.y += Math.sin(particle.direction) * speed *
                    deltaTime;
