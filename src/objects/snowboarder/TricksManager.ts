...
export class TricksManager {
    ...
    public trickStart(boardRotation: number, heading: number) {
        this.switch =
            Math.abs(ExtraMath.angleDifference(boardRotation % 360, heading)) >
                90;
        this.rotationText = TextManager.createUpdatingText(
            `Rotation`,
            `0`,
            "#FF00FF",
            2,
    ...
    private updateSpinTrick(rotationDiff: number) {
        ...
        const trickText = `${this.switch ? "Switch " : ""}${
            frontside ? "Frontside " : "Backside "
        }${Math.abs(closest90)}Spin`;
